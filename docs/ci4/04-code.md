...
- поле `include` у Vitest-конфігурації звужено до `'src/**/*.vitest.{js,vue}'` (dual-runner стратегія);
...
**`gmail_inbox_count(app, state, endpoints) -> Result<u64, GmailError>`** — викликає `acquire_access_token` та `fetch_inbox_count_at(&endpoints.gmail_label_inbox, &token)`. Повертає `messagesTotal` з `users.labels.get?id=INBOX` — канонічний шлях із одним запитом і точним числом листів INBOX (не `resultSizeEstimate`). Status mapping: 401 → `GmailError::ReauthRequired`; інші не-2xx → `GmailError::Http { status, body }`; JSON-помилки → `GmailError::Parse`.
...
`.env` (відстежується у приватному git-репозиторії):
...
- `.gitignore` містить `.claude/worktrees/` — без цього `jscpd` знаходив дублікати між робочими файлами та робочими деревами.
