...
У mlmail існує граф зв'язків ADR↔C4-проекції (дані є в `manifest.json`, sentinel-мітках та `decisions.md`), але інженери не мають зручного in-editor способу переходити ними — відкривають файли вручну. Потрібен засіб для навігації по графу безпосередньо у VS Code.
...
- **B — Foam + збагачені forward-лінки:** додатково підкрутити шаблони docs:regen, щоб проекції генерували явні `[slug](../adr/slug.md)`, тригерить повний LLM-regen.
- **C — Foam + рукотворний hub-вузол:** окремий центральний граф-файл. `docs/ci4/README.md` вже є хабом.
- **Mermaid-граф з manifest.json:** генерувати статичну діаграму в `decisions.md` — не інтерактивна, без підтримки навігації.
