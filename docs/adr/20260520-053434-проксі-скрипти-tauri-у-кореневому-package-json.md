...
Chosen option: "`bun --cwd=app run tauri ...`", because флаг `--cwd` не змінює поточну директорію оболонки (на відміну від `cd`), і підтвердився тестом `bun --cwd=app run tauri --version` → `tauri-cli 2.11.1` без жодного `cd`.

### Consequences

- Good, because кореневий `bun run dev` і `bun run tauri` резолвяться без зміни CWD оболонки — `tauri-cli 2.11.1` відповідає коректно.
