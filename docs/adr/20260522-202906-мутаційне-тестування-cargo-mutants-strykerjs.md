...
### Розширення `.gitignore` для артефактів `cargo-mutants`

`cargo-mutants` при повторному запуску перейменовує `mutants.out/` у `mutants.out.old/`. Попередній патерн `.gitignore` покривав лише `mutants.out/`, тому `mutants.out.old/` з'являлась як untracked у `git status`. Замінено на glob `mutants.out*/`, що покриває і `mutants.out/`, і `mutants.out.old/`, і будь-які майбутні ротаційні суфікси без дублювання рядків. Перевірка: `git check-ignore -v app/src-tauri/mutants.out.old/caught.txt` → `.gitignore:39:mutants.out*/`.
