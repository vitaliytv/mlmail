...
- Тримати `app/src-tauri/.env` у `.gitignore`, шерити Client IDs поза репо вручну
...
- Good, because онбординг нового розробника спрощується — Client IDs доступні з репо без окремого кроку передачі.
...
Файл `.env` закомічено у приватний репозиторій `vitaliytv/mlmail` після прибирання рядка з `.gitignore`. Репозиторій приватний (`isPrivate: true`).
