...
Проєкт `mlmail` має monorepo-структуру: кореневий `package.json` і воркспейс `app/`. Всі Tauri-скрипти (`tauri dev`, `tauri android dev`) визначені всередині `app/package.json`. Запуск `tauri dev` вимагав переходу в `app/` — з кореня прямого способу не було. Окрім того, потрібно розрізняти `mac` (desktop dev) та Android-запуск, щоб вони не конфліктували за аргументами.
...
`tauri` — загальний проксі для будь-яких Tauri-підкоманд (`tauri android dev` тощо); `mac` — фіксований аліас саме на `tauri dev` для mac-десктопа.

### Consequences
* Good, because `bun run mac` і `bun run tauri <args>` працюють з кореня без ручного `cd app`.
* Bad, because `mac` і `android` (наявний в `app/package.json`) залишаються в різних місцях — `android` ще не проксований у корінь.

## More Information
* Змінений файл: `/Users/vitaliytv/www/vitaliytv/mlmail/package.json`, рядки `"tauri"` і `"mac"`.
* Існуючий скрипт в `app/package.json`: `"android": "tauri android dev"` — не перенесено в корінь у цій сесії.
