...
### Деталі реалізації PKCE flow (macOS + Android)

**macOS:** loopback HTTP сервер на випадковому порту приймає redirect від браузера; PKCE `(verifier, challenge)` генерується у `auth/pkce.rs`; токени зберігаються через `keyring` crate → macOS Keychain.

**Android:** Kotlin `CredentialManagerFlow` → `GetSignInWithGoogleOption` (ID token); `AuthorizationFlow` через AppAuth (server auth code); `EncryptedSharedPreferences` у `SecureStore.kt`; Rust ↔ Kotlin через Tauri 2 mobile plugin (`MlmailAuthPlugin`).

**Спільне:** `token_exchange.rs` → `oauth2.googleapis.com/token`; `AuthState.expiry: Option<Instant>` + 30 с буфер freshness; 5 команд: `auth_start_login`, `auth_get_access_token`, `auth_is_authenticated`, `auth_current_email`, `auth_logout`.
