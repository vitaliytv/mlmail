...
- `with-lock.js` — обгортка з ексклюзивним локом (`.coverage.lock`, PID + перевірка живучості, реентрантна): усі команди метрик у `package.json` запускаються через неї, тож паралельні прогони неможливі.
