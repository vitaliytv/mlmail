<template>
  <!-- Plugin A2UI detail (reader slot): validated nitra.core surface + draft action -->
  <q-card flat bordered class="plugin-detail column">
    <q-card-section class="row items-center q-pb-none">
      <div class="text-subtitle2">Плагін · detail</div>
      <q-space />
      <q-spinner v-if="loading" size="18px" color="primary" />
      <q-btn v-else @click="reload" flat dense round icon="sym_o_refresh" aria-label="Оновити панель detail плагіна" />
    </q-card-section>
    <q-separator />
    <q-card-section class="col">
      <div v-if="loadError" class="text-negative text-caption">{{ loadError }}</div>
      <A2uiSurface v-else-if="surface" @action="onAction" @error="onRenderError" :surface="surface" />
      <div v-else class="text-grey-6 text-caption">Немає поверхні detail плагіна.</div>
      <div v-if="lastAction" class="text-caption text-grey-6 q-mt-sm">Остання дія: {{ lastAction }}</div>
    </q-card-section>
  </q-card>
</template>

<script setup>
/**
 * Host slot for A2UI detail: loads a Rust-validated sample surface
 * (`plugin_a2ui_sample_detail`) and routes createDraft via
 * `plugin_sidebar_create_draft` (same draft path as sidebar).
 */
import { A2uiSurface } from '@7n/tauri-components/components'
import { invoke } from '@tauri-apps/api/core'

const loading = ref(false)
const loadError = ref('')
const surface = ref(null)
const lastAction = ref('')

/** Load (or reload) the sample detail surface from the Rust plugin host. */
async function reload() {
  loading.value = true
  loadError.value = ''
  try {
    surface.value = await invoke('plugin_a2ui_sample_detail')
  } catch (error) {
    surface.value = null
    loadError.value = error?.message || String(error)
  } finally {
    loading.value = false
  }
}

/**
 * @param {object} payload the emitted action event
 */
function onAction(payload) {
  const name = payload?.action?.event?.name || 'action'
  lastAction.value = name
  if (name === 'createDraft') {
    runCreateDraft()
  }
}

/** Invoke the Rust plugin host's createDraft handler and surface its audit result. */
async function runCreateDraft() {
  loading.value = true
  loadError.value = ''
  try {
    const r = await invoke('plugin_sidebar_create_draft')
    lastAction.value = `createDraft → ${r.draftId} (${r.auditResult})`
  } catch (error) {
    loadError.value = error?.message || String(error)
  } finally {
    loading.value = false
  }
}

/**
 * @param {string} msg the render error to display
 */
function onRenderError(msg) {
  loadError.value = msg
}

onMounted(() => {
  reload()
})
</script>

<style scoped>
.plugin-detail {
  min-height: 120px;
  max-width: 100%;
}
</style>
